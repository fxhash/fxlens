...
interface ImageLoaderContextType {
  enqueueImage: (id: string, url: string, loadFn?: (url: string) => Promise<string>, priority?: number) => void;
...
export const ImageLoaderProvider: React.FC<ImageLoaderProviderProps> = ({
  ...
  const updateImageState = useCallback((id: string, update: Partial<ImageState>) => {
    setImageStates(prev => {
      const newState = {
        ...prev,
        [id]: {
  ...
  const processQueue = useCallback(async () => {
    ...
    while (queueRef.current.length > 0 && currentLoadingRef.current < maxConcurrent) {
      const item = queueRef.current.shift();
      if (item) {
        currentLoadingRef.current++;
        log(`Loading image ${item.id} from ${item.url}, current loading: ${currentLoadingRef.current}`);

        try {
          await item.load();
          log(`Successfully loaded image ${item.id}`);
        } catch (error) {
          console.error(`Error loading image ${item.id} from ${item.url}:`, error);
        } finally {
          currentLoadingRef.current--;
          log(`Finished loading image ${item.id}, current loading: ${currentLoadingRef.current}`);
        }
      }
    }
    ...
    if (queueRef.current.length > 0 && currentLoadingRef.current < maxConcurrent) {
      log(`Queue still has ${queueRef.current.length} items, continuing processing`);
      processQueue();
    } else {
  ...
  const enqueueImage = useCallback((
    ...
    const isInQueue = queueRef.current.some(item =>
      item.id === id &&
      item.url === url
    );
    ...
    const load = async () => {
      try {
        ...
        const loader = loadFn || loadersRef.current[id] || loadersRef.current._default || (
          async (url: string) => {
        ...
        updateImageState(id, { src: imageSrc, isLoading: false, error: undefined, url });
      } catch (error) {
  ...
  const dequeueImage = useCallback((id: string) => {
    const beforeLength = queueRef.current.length;
    queueRef.current = queueRef.current.filter(item => item.id !== id);
    log(`Removed image ${id} from queue. Queue size changed from ${beforeLength} to ${queueRef.current.length}`);

  }, [log]);

  const reloadImage = useCallback((id: string) => {
  ...
  const contextValue = useMemo(() => ({
    ...
    debug,
    setDebug,
  }), [enqueueImage, dequeueImage, reloadImage, getImageState, imageStates, debug, setDebug]);
...
export function useImageLoader(
  ...
  const { enqueueImage, dequeueImage, getImageState, debug } = useContext(ImageLoaderContext);
