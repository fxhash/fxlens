...
import {FxParamDefinition, FxParamOptionsMap} from "types/fxparams"
...
interface IParameterController {
  ...
  updateBinding?: ( binding: InputBindingApi<any, any>, definiton: FxParamDefinition<any>, p?: Pane, v?: ParameterValueMap) => void
}
  ...
  [EParameterType.string]: {
    ...
    controller: {
      ...
      updateBinding: (binding, definition) => {
        const controller = (binding.controller_.valueController as FxStringInputController);
  ...
  [EParameterType.number]: {
    ...
    controller: {
      ...
      updateBinding: (binding, definition, p, v) => {
        if (!p || !v) return;
        const controller = (binding.controller_.valueController as SliderTextController)
        // you cannot update the step of a number controller so we need to re-init the input
        // @ts-ignore
        if(controller.sliderController.baseStep_ !== definition.options?.step) {
          const index = p.children.findIndex((input) => (input as InputBindingApi<any,any>).controller_.binding.target.key === definition.id)
          p.remove(binding)
          ...
          controller.sliderController.props.set('minValue', definition.options?.min)
          controller.sliderController.props.set('maxValue', definition.options?.max)
...
export function createFxPane(
  ...
  Object.keys(params).forEach((key: string) => {
    ...
    const { controller } =
      parameterControlsDefinition[paramDefinition.type as EParameterType]
    const inputBinding = pane.children.find((input) => (input as InputBindingApi<any,any>).controller_.binding.target.key === key) as InputBindingApi<any, any>
    if(!inputBinding) {
